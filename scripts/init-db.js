...
const initDatabase = async () => {
  try {
    ...
    try {
      ...
      console.log("✅ Cliente Alan Brito creado")
    } catch (err) {
      if (err.code !== "ER_DUP_ENTRY") throw err
      console.log("ℹ️ Cliente Alan Brito ya existe")
    }
    ...
    try {
      ...
      console.log("✅ Cliente Zoyla Vaca creado")
    } catch (err) {
      if (err.code !== "ER_DUP_ENTRY") throw err
      console.log("ℹ️ Cliente Zoyla Vaca ya existe")
    }
    ...
    try {
      ...
      console.log("✅ Producto Perro creado")
    } catch (err) {
      if (err.code !== "ER_DUP_ENTRY") throw err
      console.log("ℹ️ Producto Perro ya existe")
    }
    ...
    try {
      ...
      console.log("✅ Producto Hamburguesa creado")
    } catch (err) {
      if (err.code !== "ER_DUP_ENTRY") throw err
      console.log("ℹ️ Producto Hamburguesa ya existe")
    }
    ...
    try {
      ...
      console.log("✅ Producto Pizza creado")
    } catch (err) {
      if (err.code !== "ER_DUP_ENTRY") throw err
      console.log("ℹ️ Producto Pizza ya existe")
    }
    ...
    console.error("   2. Verifica que MySQL esté corriendo en el puerto 3306")
    console.error("   3. Revisa las credenciales en el archivo .env")
    console.error("   4. Si no tienes MySQL, usa la alternativa SQLite")
    process.exit(1)
  }
}
